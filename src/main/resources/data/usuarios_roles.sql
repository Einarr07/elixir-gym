...
SELECT 'ADMIN', 'Administrador del sistema con acceso completo a todas las funcionalidades y datos.'
    WHERE NOT EXISTS (SELECT 1 FROM roles WHERE nombre_rol = 'ADMIN');

INSERT INTO roles(nombre_rol, descripcion_rol)
SELECT 'ENTRENADOR', 'Profesional encargado de crear planes de entrenamiento y guiar a los clientes.'
    WHERE NOT EXISTS (SELECT 1 FROM roles WHERE nombre_rol = 'ENTRENADOR');

INSERT INTO roles(nombre_rol, descripcion_rol)
SELECT 'CLIENTE', 'Usuario que accede al sistema para gestionar y seguir sus planes de entrenamiento.'
...
FROM (VALUES
          ...
          ('CLIENTE', 'valentina@example.com')
     ) AS datos(rol_nombre, usuario_email)
         JOIN roles r ON r.nombre_rol = datos.rol_nombre
         JOIN usuarios u ON u.email = datos.usuario_email
WHERE NOT EXISTS (
    SELECT 1 FROM usuarios_roles ur
    WHERE ur.id_rol = r.id_rol AND ur.id_usuario = u.id_usuario
);